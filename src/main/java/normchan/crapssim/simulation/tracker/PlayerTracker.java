...
public class PlayerTracker implements Tracker, Observer {
	...
	public PlayerTracker(GameManager manager, int initialBalance) {
		...
		gameManager.getPlayer().setTracker(this);
	}
	...
	public void printResults(PrintStream stream) {
		if (stats.mean > 4950) {
			...
			formatter.format(gameManager.getPlayer().getStrategy() + "\nPlayer ended with an average balance of $%.2f (started with $%d) simulationCount %d\n", stats.mean,
					initialBalance, simulationCount);
